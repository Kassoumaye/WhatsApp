...
const Status = () => {
  ...
  <View style={styles.container}>
    ...
    <View>
      <Image source ={require('../../assets/kimberly.jpg')} style={styles.image}/>
    </View>
    <Text style={styles.myStatusHeading}>My status</Text>
    <Text style={styles.myStatusSubtext}>Tap to add status update</Text>

    </View>
    <FlatList data={combinedStatusData} keyExtractor={(item, index) => index.toString()} renderItem={({item}) => (
      <>
        <Text style={styles.viewedStatus}>{item.title}</Text>
        <FlatList data={item.data} keyExtractor={(item) => item.id.toString()} renderItem={({item}) => (
